...
The `homelab` NFS share is hosted on the Zyxel NAS326 and is currently restricted to `lab-core01` (`<PRIVATE_IP>`).
...
```text
<PRIVATE_IP>:/i-data/cfb9d897/nfs/homelab -> /mnt/nas
```
...
```text
<PRIVATE_IP>:/i-data/cfb9d897/photo -> /mnt/photo-library
```

The photo export is restricted to `<PRIVATE_IP>` and uses NFSv3.
